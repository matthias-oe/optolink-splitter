...
from ha_shared_config import datapoints
...
def verify_mqtt_optolink_lwt(timeout=10):
    ...
    LWT_TOPIC = settings_ini.mqtt_topic + "/LWT" 
...
def beautify(text):
    result = text
    if "beautifier" in datapoints:
        beautifier = datapoints["beautifier"]
        if "search" in beautifier and "replace" in beautifier:
            sea = datapoints["beautifier"]["search"]
            rep = datapoints["beautifier"]["replace"]
            i = 0
            while i < len(sea):
...
def publish_ha_discovery():
    ...
    mqtt_base = settings_ini.mqtt_topic
    ha_prefix = "homeassistant"
    
    # Device-Info
    node_id = datapoints["node_id"]
    device = datapoints["device"]
    
    total_published = 0
    
    for domain_config in datapoints["domains"]:
            ...
            for key, value in list(discovery_config.items()):
                if key.endswith("_template") and isinstance(value, str):
                    if "command_topic" not in discovery_config:
                        discovery_config["command_topic"] = settings_ini.mqtt_listen
